...
#define match(x,y) if (wstring(argv[x]).length()<=wstring(y).length() ? wstring(argv[x])==wstring(y).substr(0,wstring(argv[x]).length()) : false)
...
void initkeydef()
{
	...
	WMDEF(WM_MOUSEMOVE); WMDEF(WM_LBUTTONDOWN); WMDEF(WM_LBUTTONUP); WMDEF(WM_LBUTTONDBLCLK); WMDEF(WM_RBUTTONDOWN); WMDEF(WM_RBUTTONUP); WMDEF(WM_RBUTTONDBLCLK); WMDEF(WM_MBUTTONDOWN); WMDEF(WM_MBUTTONUP); WMDEF(WM_MBUTTONDBLCLK); WMDEF(WM_MOUSEWHEEL); WMDEF(WM_DESTROY); WMDEF(WM_KEYDOWN); WMDEF(WM_KEYUP);
...
LRESULT CALLBACK WindowProc(_In_  HWND hwnd, _In_  UINT uMsg, _In_  WPARAM wParam, _In_  LPARAM lParam)
{
	...
	case  WM_MOUSEWHEEL:
		...
		break;
	case WM_MOUSEMOVE:
		strinfo += L" " + wmmap[uMsg] + L"." + to_wstring(LOWORD(lParam)) + L"." + to_wstring(HIWORD(lParam)) + L"." + query(getres((wchar_t*)windowtag.c_str())->region.p, LOWORD(lParam), HIWORD(lParam));
		break;
	case WM_PAINT:
	...
	if (strinfo.length() > 8192) {
		strinfo = strinfo.substr(strinfo.length()-8192, 8192);
		int strstart = 0;
...
void image(wchar_t *CmdLine)
{
	...
	LocalFree(argv);
	SetEnvironmentVariableW(L"errorlevel", 0);
	return;
}
